...
# Script de verificación de estructura de documentación para sicora-be-python
# Aplica las mismas reglas que sicora-app, sicora-app-fe y sicora-be-go
...
verify_structure() {
    ...
    # Verificar scripts en la raíz (algunos específicos de Python están permitidos)
    ...
    for file in "${script_files[@]}"; do
        local basename=$(basename "$file")
        # Permitir algunos scripts específicos del backend Python
        if [[ "$basename" != "start_services.sh" && "$basename" != "deploy.sh" && "$basename" != "build.sh" ]]; then
...
auto_organize() {
    ...
    local categories=("integracion" "configuracion" "desarrollo" "reportes" "microservicios" "ia" "apis")
    ...
    for file in "${md_files[@]}"; do
        ...
        case "$basename" in
            *INTEGRACION*|*INTEGRATION*)
                ...
                ;;
            *AISERVICE*|*AI_SERVICE*|*KBSERVICE*|*KB_SERVICE*)
                category="ia"
                ;;
    ...
    # Mover scripts .sh a carpeta scripts (excepto los permitidos)
    local script_files=($(find "$PROJECT_ROOT" -maxdepth 1 -name "*.sh" -type f))
...
# ${category^} - Backend Python SICORA

## Documentación de ${category^}

Esta carpeta contiene la documentación relacionada con ${category} del backend Python SICORA.
...
generate_report() {
    local report_file="$PROJECT_ROOT/_docs/reportes/VERIFICACION_ESTRUCTURA_DOCUMENTACION_BE_PYTHON.md"
    
    # Crear carpeta reportes si no existe
    mkdir -p "$PROJECT_ROOT/_docs/reportes"
    
    cat > "$report_file" << EOF
# Reporte de Verificación - Estructura de Documentación Backend Python SICORA
...
EOF
    ...
    local categories=("integracion" "configuracion" "desarrollo" "reportes" "microservicios" "ia" "apis" "general")
    for category in "${categories[@]}"; do
        if [ -d "$PROJECT_ROOT/_docs/$category" ]; then
...
# Función principal
main() {
    echo -e "${BLUE}🔍 Verificación de Estructura de Documentación SICORA Backend Python${NC}"
    echo "====================================================================="
