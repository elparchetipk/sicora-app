...
class MessageCreate(BaseModel):
    ...
    message_type: MessageTypeEnum = Field(default=MessageTypeEnum.USER, description="Type of message")
...
class ConversationCreate(BaseModel):
    ...
    model_id: Optional[UUID] = Field(default=None, description="AI model to use for this conversation")
    system_prompt: Optional[str] = Field(default=None, max_length=10000, description="System prompt for the conversation")
...
class ConversationUpdate(BaseModel):
    ...
    status: Optional[ConversationStatusEnum] = Field(default=None, description="New conversation status")
...
class ConversationWithMessages(ConversationResponse):
    ...
    messages: List[MessageResponse] = Field(default=[], description="Messages in the conversation")
...
class ChatResponse(BaseModel):
    ...
    model_info: Optional[Dict[str, Any]] = Field(default=None, description="Model information used")
...
class ConversationListQuery(BaseModel):
    ...
    status: Optional[ConversationStatusEnum] = Field(default=None, description="Filter by status")
    limit: int = Field(default=20, ge=1, le=100, description="Maximum number of conversations to return")
...
class EnhancedChatRequest(BaseModel):
    ...
    search_categories: Optional[List[str]] = Field(None, description="Specific categories to search")
    context_limit: int = Field(default=5, ge=1, le=10, description="Max context items from KB")
...
class EnhancedChatResponse(BaseModel):
    ...
    knowledge_sources_used: int = Field(default=0, description="Number of KB sources used")
    context_categories: List[str] = Field(default_factory=list, description="Categories found in context")
...
class KnowledgeSearchRequest(BaseModel):
    ...
    search_type: str = Field(default="hybrid", pattern="^(text|semantic|hybrid)$", description="Type of search")
    category: Optional[str] = Field(None, description="Category filter")
    limit: int = Field(default=5, ge=1, le=20, description="Max number of results")
    search_regulatory: bool = Field(default=False, description="Search specifically in regulatory content")
