...
class AnalyticsQuery(BaseModel):
    ...
    period: AnalyticsPeriod = Field(default=AnalyticsPeriod.DAY, description="Aggregation period")
    metrics: List[str] = Field(..., min_length=1, description="List of metrics to calculate")
...
class MetricValue(BaseModel):
    ...
    change_percentage: Optional[float] = Field(default=None, description="Percentage change from previous period")


class TimeSeriesDataPoint(BaseModel):
    """Schema for time series data point."""
    timestamp: datetime = Field(..., description="Data point timestamp")
    metrics: List[MetricValue] = Field(..., description="Metric values at this timestamp")
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="Additional data point metadata")
...
class ConversationAnalytics(BaseModel):
    ...
    avg_conversation_length: float = Field(..., description="Average conversation length in messages")
    avg_conversation_duration_minutes: float = Field(..., description="Average conversation duration")
    conversations_by_model: Dict[str, int] = Field(..., description="Conversations count by model")
    conversations_by_day: List[Dict[str, Any]] = Field(..., description="Daily conversation counts")
...
class MessageAnalytics(BaseModel):
    ...
    messages_by_hour: List[Dict[str, Any]] = Field(..., description="Hourly message distribution")
    sentiment_distribution: Dict[str, int] = Field(..., description="Message sentiment distribution")
...
class ModelAnalytics(BaseModel):
    ...
    error_rate_by_model: Dict[str, float] = Field(..., description="Error rates by model")
...
class KnowledgeAnalytics(BaseModel):
    ...
    entries_by_category: Dict[str, int] = Field(..., description="Entries count by category")
    most_searched_topics: List[Dict[str, Any]] = Field(..., description="Most searched topics")
    search_success_rate: float = Field(..., description="Search success rate")
    avg_search_results: float = Field(..., description="Average number of search results")
    entries_created_trend: List[Dict[str, Any]] = Field(..., description="Knowledge entry creation trend")
    top_contributors: List[Dict[str, Any]] = Field(..., description="Top knowledge contributors")
...
class UserAnalytics(BaseModel):
    ...
    usage_by_user_type: Dict[str, int] = Field(..., description="Usage distribution by user type")
...
class SystemAnalytics(BaseModel):
    ...
    database_performance: Dict[str, float] = Field(..., description="Database performance metrics")
...
class ComprehensiveAnalytics(BaseModel):
    ...
    conversation_analytics: ConversationAnalytics = Field(..., description="Conversation metrics")
    ...
    knowledge_analytics: KnowledgeAnalytics = Field(..., description="Knowledge base metrics")
    user_analytics: UserAnalytics = Field(..., description="User behavior metrics")
    system_analytics: SystemAnalytics = Field(..., description="System performance metrics")
...
class AlertThreshold(BaseModel):
    ...
    notification_channels: List[str] = Field(..., description="Channels to send notifications")
...
class CustomReport(BaseModel):
    ...
    schedule: Optional[str] = Field(default=None, description="Cron schedule for automated generation")
    recipients: List[str] = Field(default=[], description="Email recipients for scheduled reports")
...
class ReportExecution(BaseModel):
    ...
    data: Optional[AnalyticsResponse] = Field(default=None, description="Report data if successful")
    error_message: Optional[str] = Field(default=None, description="Error message if failed")
    started_at: datetime = Field(..., description="Execution start time")
    completed_at: Optional[datetime] = Field(default=None, description="Execution completion time")
    execution_time_ms: Optional[float] = Field(default=None, description="Execution duration")
