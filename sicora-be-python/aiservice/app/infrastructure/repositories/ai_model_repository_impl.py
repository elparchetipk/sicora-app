...
class SQLAlchemyAIModelRepository(AIModelRepository):
    ...
    async def get_by_id(self, model_id: UUID) -> Optional[AIModel]:
        ...
        try:
            ...
            logger.error(f"Error getting AI model by ID {model_id}: {str(e)}", exc_info=True)
            return None
    ...
    async def get_by_name(self, name: str) -> Optional[AIModel]:
        ...
        try:
            ...
            logger.error(f"Error getting AI model by name {name}: {str(e)}", exc_info=True)
            return None
    ...
    async def get_by_type(self, model_type: ModelType) -> List[AIModel]:
        ...
        try:
            ...
            logger.error(f"Error getting AI models by type {model_type}: {str(e)}", exc_info=True)
            return []
    ...
    async def get_active_models(self) -> List[AIModel]:
        ...
        try:
            stmt = select(AIModelModel).where(
                and_(
                    AIModelModel.status == ModelStatus.ACTIVE.value,
                    AIModelModel.is_active == True
                )
            ).order_by(AIModelModel.name)
    ...
    async def update(self, model: AIModel) -> AIModel:
        ...
        try:
            stmt = update(AIModelModel).where(
                AIModelModel.id == model.model_id
            ).values(
                name=model.name,
                model_type=model.model_type.value,
                model_name=model.model_name,
    ...
    async def get_models_by_feature(self, feature: str) -> List[AIModel]:
        ...
        try:
            ...
            stmt = select(AIModelModel).where(
                and_(
                    func.json_extract(AIModelModel.supported_features, '$').like(f'%"{feature}"%'),
                    AIModelModel.is_active == True,
                    AIModelModel.status == ModelStatus.ACTIVE.value
                )
            ).order_by(AIModelModel.name)
            ...
            logger.error(f"Error getting models by feature {feature}: {str(e)}", exc_info=True)
            return []
    ...
    async def get_default_model(self, model_type: Optional[ModelType] = None) -> Optional[AIModel]:
        ...
        try:
            ...
            stmt = select(AIModelModel).where(
                and_(*conditions)
            ).order_by(AIModelModel.name).limit(1)
    ...
    async def get_model_statistics(self) -> dict:
        ...
        try:
            ...
            # Models by type
            type_stmt = select(
                AIModelModel.model_type,
                func.count(AIModelModel.id)
            ).where(
                AIModelModel.is_active == True
