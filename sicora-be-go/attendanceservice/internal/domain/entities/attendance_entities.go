...
const (
	...
	AttendanceStatusJustified AttendanceStatus = "JUSTIFIED"
...
const (
	...
	QRCodeStatusUsed    QRCodeStatus = "USED"
)
...
const (
	JustificationStatusPending  JustificationStatus = "PENDING"
	JustificationStatusApproved JustificationStatus = "APPROVED"
	JustificationStatusRejected JustificationStatus = "REJECTED"
)
...
const (
	...
	AlertTypeLate        AlertType = "LATE"
	AlertTypePattern     AlertType = "PATTERN"
	AlertTypeConsecutive AlertType = "CONSECUTIVE"
...
func (ar *AttendanceRecord) IsValidStatus() bool {
	switch ar.Status {
	case AttendanceStatusPresent, AttendanceStatusAbsent, AttendanceStatusJustified, AttendanceStatusLate:
		return true
	default:
		return false
	}
...
func (j *Justification) IsValidJustificationStatus() bool {
	switch j.Status {
	case JustificationStatusPending, JustificationStatusApproved, JustificationStatusRejected:
		return true
	default:
		return false
	}
...
func (aa *AttendanceAlert) IsValidAlertType() bool {
	switch aa.Type {
	case AlertTypeAbsence, AlertTypeLate, AlertTypePattern, AlertTypeConsecutive, AlertTypePercentage, AlertTypeCustom:
		return true
	default:
		return false
	}
...
func (aa *AttendanceAlert) IsCritical() bool {
	return aa.Level == AlertLevelCritical
}
...
func (j *Justification) Approve(reviewerID uuid.UUID) {
	now := time.Now()
	j.Status = JustificationStatusApproved
...
func (j *Justification) Reject(reviewerID uuid.UUID, comments string) {
	now := time.Now()
	j.Status = JustificationStatusRejected
	j.ReviewedBy = &reviewerID
	j.ReviewDate = &now
