...
const (
	FAQStatusDraft     FAQStatus = "DRAFT"
	FAQStatusReview    FAQStatus = "REVIEW"
	FAQStatusPublished FAQStatus = "PUBLISHED"
	FAQStatusArchived  FAQStatus = "ARCHIVED"
)
...
const (
	FAQPriorityLow    FAQPriority = "LOW"
	FAQPriorityMedium FAQPriority = "MEDIUM"
	FAQPriorityHigh   FAQPriority = "HIGH"
	FAQPriorityUrgent FAQPriority = "URGENT"
)
...
func (f *FAQ) IsPublished() bool {
	return f.Status == FAQStatusPublished
}
...
func (f *FAQ) CanBeEditedBy(userID uuid.UUID, userRole string) bool {
	...
	if f.AuthorID == userID {
		if f.Status != FAQStatusPublished || userRole == "ADMIN" || userRole == "KB_ADMIN" {
			return true
		}
	}
	...
	if f.ReviewerID != nil && *f.ReviewerID == userID && f.Status == FAQStatusReview {
		return true
	}
